...
namespace Phases
{
    ...
    */
    ...
    public class InstructionManager : MonoBehaviour
    {
        ...
        [Space] [Header("Instruction Objects")] [Space]
        [SerializeField] Button instructionReminder; //Show again the instruction

        [Space] [Header("Fading Variables")] [Space]
        ...
        public void SetInstructionReminder(ButtonClick bt){
            instructionReminder.onClick.AddListener(delegate{ bt(); }); 
        }

        public void ButtonChangeOfScaleAnimation(Vector3 scale, float time)
        {
            Util.ChangeScaleAnimation(instructionReminder.GetComponent<RectTransform>(), scale, time);
        }
